  ...
  db.query("SELECT secretKey FROM myKeys WHERE secretKey = ?", [req.body.secretKey], (err, result) => {
    ...
    else 
    {
      ...
      db.query(
        ...
        [body.title, body.description, body.date, body.link, body.github, body.githubAPI, body.dev, body.frontendTech, body.backendTech], 
        (err) => {
          if(err !== null){
            rep.send({error: err.message});
          }
  ...
  db.query("SELECT secretKey FROM myKeys WHERE secretKey = ?", [req.body.secretKey], (err, result) => {
    ...
    else 
    {
      ...
      db.query(
        ...
        [body.title, body.description, body.date, body.link, body.github, body.githubAPI, body.dev, body.frontendTech, body.backendTech, req.params.id], 
        (err) => {
          if(err !== null){
            rep.send({error: err.message});
          }
